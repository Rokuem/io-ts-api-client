...
describe("Operation", () => {
  ...
  test("Should have correct type for the payload constructor", () => {
    expectTypeOf(operation.payloadConstructor).parameters.toMatchTypeOf([
      operation.options,
    ]);
    expectTypeOf(operation.payloadConstructor).returns.toMatchTypeOf(
      operation.payloadModel?.tsInterface
    );
  });
});
